...
@pytest.fixture(scope="module")
def builder():
    return _load_module("build_status_issue")
...
# =========================================================================== #
# build_status_issue.py -- four-state snapshot assembly
# =========================================================================== #

class TestBuildBody:
    """
    Spec unit 3: assemble the status-issue body from drift_result + suggestions.
    Snapshot-style assertions for the four states. The non-negotiable invariant
    across ALL states: no bare numeric score appears, and the suggestions section
    always carries the 'no scores applied' wording.

    build_body(drift, suggestions, drift_ts, suggest_ts) -- pure, no I/O/clock.
    """

    DRIFT_TS = "2026-05-29T06:00:00Z"
    SUGGEST_TS = "2026-05-29T06:00:00Z"

    # --- canned inputs -------------------------------------------------------- #

    def _drift_present(self):
        return {
            "has_drift": True,
            "stale_sonarr": ["11111111111111111111111111111111"],
            "stale_radarr": [],
            "new_schema_props": ["radarr.*.quality_profiles.upgrade.until_score"],
            "issue_body": "## Stale trash_ids\n\n- `1111...`",
        }

    def _drift_clean(self):
        return {
            "has_drift": False,
            "stale_sonarr": [],
            "stale_radarr": [],
    ...
    def _suggestions_present(self):
        return {
            "generated_for_config_hash": "deadbeef",
            "sonarr": [
                {"trash_id": "44444444444444444444444444444444",
                 "name": "Remux Tier 01", "category": "Remux",
                 "why_it_fits": "fits your remux 1080p profile",
            ...
            "radarr": [
                {"trash_id": "66666666666666666666666666666666",
                 "name": "IMAX Enhanced", "category": "Misc",
                 "why_it_fits": "matches your 4K intent",
                 "confidence": "low"},
            ],
        }

    def _suggestions_empty(self):
        return {}

    def _build(self, builder, drift, suggestions):
        return builder.build_body(drift, suggestions, self.DRIFT_TS,
                                  self.SUGGEST_TS)

    # --- shared invariant ----------------------------------------------------- #

    def _assert_no_scores_invariant(self, body):
        low = body.lower()
        # The advisory disclaimer must always be present in the suggestions section.
        assert "no scores applied" in low, (
            "suggestions section must carry the 'no scores applied' disclaimer"
        )
        # Strip tokens that legitimately contain digits before scanning for a bare
        # integer that could be misread as a score: 32-hex trash_ids, resolution
        # tiers (1080p / 2160p / 720p), and '4k'. A schema PROPERTY name like
        # 'until_score' is a path, not an applied score -- the substring "score"
        # in such a path is acceptable; only bare integers are forbidden.
        scrubbed = re.sub(r"\b[0-9a-fA-F]{32}\b", "", body)
        scrubbed = re.sub(r"\b\d{3,4}p\b", "", scrubbed, flags=re.IGNORECASE)
        scrubbed = re.sub(r"\b4k\b", "", scrubbed, flags=re.IGNORECASE)
        # Drop the ISO timestamps in the footer (they contain digits but are not
        # scores).
        scrubbed = re.sub(r"\d{4}-\d{2}-\d{2}T[\d:]+Z", "", scrubbed)
        assert _looks_score_free(scrubbed), (
            "no applied-score-shaped value may appear in the body:\n" + scrubbed
        )

    # --- the four states ------------------------------------------------------ #

    def test_state_drift_only(self, builder):
        body = self._build(builder, self._drift_present(),
                           self._suggestions_empty())
        low = body.lower()
        assert "## must-fix drift" in low
        assert "stale" in low
        # No suggestions -> the empty-state call to action.
        assert "add quality profiles/cfs to recyclarr.yml to get suggestions" in low
        self._assert_no_scores_invariant(body)

    def test_state_suggestions_only(self, builder):
        body = self._build(builder, self._drift_clean(),
                           self._suggestions_present())
        low = body.lower()
        # Drift section present but clean.
        assert "no drift detected" in low
        # Suggestions section renders the candidate names + confidence.
        assert "remux tier 01" in low
        assert "imax enhanced" in low
        assert "confidence: high" in low and "confidence: low" in low
        self._assert_no_scores_invariant(body)

    def test_state_both(self, builder):
        body = self._build(builder, self._drift_present(),
                           self._suggestions_present())
        low = body.lower()
        assert "## must-fix drift" in low
        assert "stale" in low
        assert "remux tier 01" in low
        assert "## suggestions (advisory" in low
        self._assert_no_scores_invariant(body)

    def test_state_neither(self, builder):
        body = self._build(builder, self._drift_clean(),
                           self._suggestions_empty())
        low = body.lower()
        assert "no drift detected" in low
    ...
    def test_section_headers_always_present(self, builder):
        # Both top-level section headers must always render so the issue layout is
        # stable across every state.
        for drift in (self._drift_present(), self._drift_clean()):
            for sugg in (self._suggestions_present(), self._suggestions_empty()):
                body = self._build(builder, drift, sugg).lower()
                assert "## must-fix drift" in body
                assert "## suggestions (advisory — no scores applied)" in body

    def test_suggestions_ranked_high_before_low(self, builder):
        # Within the body, a high-confidence suggestion must appear before a
        # low-confidence one (spec: sorted by confidence desc).
        suggestions = {
            "sonarr": [
                {"trash_id": "a" * 32, "name": "Low One", "category": "c",
                 "why_it_fits": "x", "confidence": "low"},
                {"trash_id": "b" * 32, "name": "High One", "category": "c",
                 "why_it_fits": "x", "confidence": "high"},
            ],
            "radarr": [],
        }
        body = self._build(builder, self._drift_clean(), suggestions)
        assert body.index("High One") < body.index("Low One")
